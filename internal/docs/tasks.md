...
| 42.10 | 真机复验：五检查点（无注入 cron exit 2 零删除 / symlink 逃逸零删除 / gate+json 全解析+failed 可见 / cp936 非 ASCII 不崩 / 常规 gate 回归，见 scr-042-plan.md 六） | 42.9 | Done (2026-08-29: 五检查点 + 部署冒烟整体 PASS 无 FAIL——本地拷贝部署 0.6.2 至 default 档案（plugins list: dir-whip enabled 0.6.2 user）+ venv 直调已部署副本 + TERMINAL_CWD 注入 + -z oneshot 真会话；CK1 rc=2 零删除零 stdout + verbatim 拒绝行；CK2 双 symlink 形态外部内容零删除；CK3 stdout 恰两行合法 JSON + 锁死项 failed=1 可见；CK4 cp936 无崩溃 + create 中文任务名正常；CK5 违规+清理混合 wakeAgent {true,1,1,0} 契约一致；冒烟=守卫拦截根写 → agent 收两步确认载荷 → 根文件确认不存在；报告=realhost-verification-report.md（v0.6.1 版已在 archive/v0.6.1/）；观察项：只读检查跟随 symlink（删除边界已闭合，低优先另议）/ oneshot 无确认通道（SCR-041 已知）/ 桌面长驻需重启加载——**Release Done (2026-08-29)**：tag v0.6.2 + GitHub Release（Latest）已推送，0.6.2_dev 合回 main（fast-forward）后删除，报告归档 archive/v0.6.2/；唯一长期分支=main) |
...
## Phase 12 — SCR-043 v0.6.3 分类链定稿 + 值域门禁 + 隔离区迁址 + gate 零自动删除 (43.x)

TDD per task: write the mapped tests FIRST (red), then implement (green).
Acceptance = mapped testing-standards §7.13 matrix (v2.11) all green + KPI
column. Active branch: `0.6.3_dev` (from `main@9ce0afa` v0.6.2; cut at
implementation start).
Design source: feedback/14（分类链与定时治理设计定稿，2026-08-30：C1-C4 +
新发现 2 项 + G1，无安全高危项）+ `docs/scr-043-plan.md`——R1-R6 定稿设计
（E1-E6 全收合一；版本定档 v0.6.3 patch，用户 2026-08-30）。建议执行顺序
R4 → R1 → R3（E2 段边界是 T1 层前置）。
