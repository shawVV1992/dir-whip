...
| 2026-08-29 | v0.6.2 pre-registration (SCR-042, docs-only): spec EN/ZH v2.9->v2.10 ACTIVE (3.4 gate-failure extension / 4.2 wakeAgent payload + case parity + symlink scan boundary / 4.4 hermes-home fallback / 7.2 rows), `docs/scr-042-plan.md` (R1-R5), spec-changes SCR-042 row, AGENTS.md sync, feedback/13 disposition note. Acceptance matrix §7.12 (6 rows, all Pending, mapped to tasks 42.R1.*-42.R5.*). Rows flip to Done as impl tasks complete | SCR-042 (2026-08-29 scope ruling: 9 items = 2 must-fix + 5 recommended + 2 free-riders, per-item benefit review) |
| 2026-08-30 | v0.6.3 pre-registration (SCR-043, docs-only): spec EN/ZH v2.10->v2.11 ACTIVE (3.4 gate contract narrowed to pure audit + wake / 4.2 two-key payload + inventory + .hermes exemption removal / 5.3 T0-T4 chain + emit-layer routing / 5.11 value-domain gating + segment-boundary matching / 5.18 quarantine relocation + message paths / 5.7 list quarantine line / 7.2/7.3 rows / 8.1 zero-auto-delete red line), `docs/scr-043-plan.md` (R1-R6), ADR-0011, spec-changes SCR-043 row, feedback/14 disposition note. Acceptance matrix §7.13 (7 rows, all Pending, mapped to tasks 43.R1.*-43.R6.*), including the SCR-042 assertion rewrite/retirement map (42.R1 deletion assertions -> inventory assertions or retired; 42.R4 four-key -> two-key). Rows flip to Done as impl tasks complete | SCR-043 (2026-08-30: feedback/14 design finalization; version ruling v0.6.3 patch per user, SCR-042 precedent) |
...
| # | Criterion (spec ref) | Mapped tests | Task | Status |
|---|----------------------|--------------|------|--------|
| 1 | Re-scan semantics: a runtime-allowlist entry does NOT settle a recorded violation — pending stays, nudge keeps firing (attempt ordinals continue); config allowlist files/dirs + session-dir still settle (5.18 R1) | TestSettleSelfheal (re-scan cases) | 41.R1.2 | Done (2026-08-29) |
| 2 | Pre-write sanction regression: a path exempted BEFORE the write creates no violation (audit diff classifies at write time) — legitimate flow unchanged (5.11/5.18 R1) | TestSettleSelfheal / TestAllowlistCommands | 41.R1.2 | Done (2026-08-29) |
| 3 | Subagent rejection: allow_path from a subagent session returns the parent-guidance variant verbatim; stats row `block/allow-path/allow-path-subagent-rejected` recorded; bus-skipped (no generic blocked fanout) (5.11 R2) | TestAllowlistCommands | 41.R2.1 | Done (2026-08-29) |
| 4 | Root rejection: allow_path(working_dir_root) returns the root-rejection message verbatim; no state change; stats row `block/allow-path/allow-path-root-rejected` recorded, bus-skipped (5.11 R2) | TestAllowlistCommands | 41.R2.1 | Done (2026-08-29) |
| 5 | Two-step confirmation: first call (no confirm) returns the confirmation payload verbatim WITHOUT adding + records confirmation-issued; confirm=true honored only for an already-briefed path; unbriefed confirm=true adds NOTHING and re-issues the payload (equivalent to a first call — never adds on its own); success fires the regular runtime-allowlist-add stats/bus; payload issuance and unbriefed-confirm re-issue logged to dir-whip.log DEBUG only (no stats/bus); latch-context conditional line appended when pending non-empty (5.11 R3) | TestAllowlistCommands | 41.R3.1 | Done (2026-08-29) |
| 6 | L1/L3/nudge message verbatim locks: config option re-attributed to the USER ("ask the user to add") + latch-period freeze explicit sentence + exact-command instruction (/dir-whip allow <path>); nudge tail carries the resolution-choice presentation ("Finish only after settlement or the user's decision"); subagent variants and settle lines unchanged (5.18 R4) | TestConfigSurface / TestSettleSelfheal | 41.R4.1 | Done (2026-08-29) |
| 7 | Full suite green at baseline (644 passed / 5 skipped re-measured at 40.9; SCR-041 additions on top) | full suite | 41.9 | Done (2026-08-29: 664 passed / 5 skipped / 0 failed) |

## 7.12 v0.6.2 — SCR-042 acceptance matrix (spec v2.10)

Maps 1:1 to tasks.md Phase 11 (42.R1.* .. 42.R5.*) and the test files
test_audit_workspace.py (extended: deletion surface / case parity / gate
output contract / encoding) / test_create_session_dir.py (extended:
absolute-path output + import shadow) / test_workspace_resolver.py
(extended: hermes home fallback).

| # | Criterion (spec ref) | Mapped tests | Task | Status |
|---|----------------------|--------------|------|--------|
| 1 | Deletion surface: --gate + resolution failure -> exit 2, empty stdout (no wakeAgent line), nothing deleted; interactive fail-open unchanged; find_tmp_entries skips symlinked session dirs AND symlinked .tmp dirs (zero scan, zero deletion, external content untouched); real-dir cleanup regression intact (3.4/4.2 R1) | TestAuditWorkspace (deletion-surface cases) | 42.R1.1/42.R1.2 | Done (2026-08-29) |
| 2 | Import hardening: both scripts load the bundled workspace_resolver via importlib from the __file__-derived absolute path (sys.modules registered); a CWD shadow workspace_resolver.py (sentinel side effect) never executes under any invocation form; workspace_resolver.__file__ == bundled path (4 R2) | TestAuditWorkspace / TestCreateSessionDir (shadow cases) | 42.R2.1 | Done (2026-08-29) |
| 3 | Case parity: root-file allowlist matching reuses the resolver's Windows-casefold matcher (README.TXT vs readme.txt consistent with allowlist.is_allowlist_file); Outputs blacklist comparisons case-insensitive (Thumbs.DB / __PYCACHE__ flagged); .hermes exemption casefolded on Windows (.HERMES not flagged) (4.2 R3) | TestAuditWorkspace (parity cases) | 42.R3.1 | Done (2026-08-29) |
| 4 | Gate output contract: wakeAgent line carries four keys always ({wakeAgent, violations, removed, failed}); gate+json stdout parses line-by-line as JSON (no plain-text removal report; failure details on stderr); injected cleanup failure -> failed=1 with removed reduced; plain gate mode keeps the text report (4.2 R4) | TestAuditWorkspace (contract cases) | 42.R4.1 | Done (2026-08-29) |
| 5 | Robustness smalls: PYTHONIOENCODING=cp936 + non-ASCII paths -> no traceback in all three scripts; create_session_dir prints an absolute path when working_dir_root is relative; hermes home falls back to the user home when LOCALAPPDATA is unset/empty (never CWD-relative) (4.1/4.4 R5) | TestAuditWorkspace / TestCreateSessionDir / TestWorkspaceResolver | 42.R5.1 | Done (2026-08-29) |
| 6 | Full suite green at baseline (664 passed / 5 skipped re-measured at 41.9; SCR-042 additions on top) | full suite | 42.9 | Done (2026-08-29: 687 passed / 5 skipped / 0 failed) |

## 7.13 v0.6.3 — SCR-043 acceptance matrix (spec v2.11)

Maps 1:1 to tasks.md Phase 12 (43.R1.* .. 43.R6.*) and the test files
test_dir_whip.py (TestClassifyTarget extended + allow_path handler) /
test_observability.py (emit routing) / test_config.py (segment boundary +
add-layer assertion + empty rejection) / test_settle_selfheal.py +
test_config_surface.py (quarantine relocation + message verbatim) /
test_audit_workspace.py (gate zero-delete + two-key payload + SCR-042
assertion rewrite/retirement).

| # | Criterion (spec ref) | Mapped tests | Task | Status |
|---|----------------------|--------------|------|--------|
| 1 | T0-T4 chain order: outside-root + runtime entry -> external-write (NOT runtime-allowlist); root itself -> block root-file even when its basename matches an allowlist file; under-root order matrix (runtime-allowlist / tier0-allowlist / allowed-file / session-dir) unchanged; settlement re-scan config-only semantics unchanged (5.3 R1) | TestClassifyTarget (extended) | 43.R1.1/43.R1.2 | Done (2026-08-30) |
| 2 | external-write routing is geometric at the emit layer: outcome=allow + outside-root target -> INFO log + external-write bus fanout; outcome string stays the fallback when the root is unresolved; allow-path rejections and runtime-allowlist-add produce no new fanout (7 emits unchanged) (5.3 R2) | TestObservability (extended) | 43.R2.1 | Done (2026-08-30) |
| 3 | Registration gating: outside-root path rejected with the self-explaining verbatim + stats `block/allow-path/allow-path-external-rejected` bus-skipped; empty/None path rejected at the add layer; `runtime_allowlist_add(path, working_dir_root)` rejects outside-root when root is injected and keeps rootless direct callers unchanged; in-root two-step confirmation flow regression (5.11 R3) | TestAllowlistCommands / TestConfig (extended) | 43.R3.1 | Done (2026-08-30) |
| 4 | Segment-boundary matching: `docs` does not exempt `docs_secret/x.txt`; exact entry hit and subpath hit unchanged; trailing-slash entries compatible (5.11 R4) | TestConfig (extended) | 43.R4.1 | Done (2026-08-30) |
| 5 | Quarantine relocation: settle moves into `<dir-whip home>/audit-quarantine/<ts>/` (outside the workspace root); L1 notice / nudge messages name the new path (verbatim rewrites of the SCR-041 R4 locks); audit flags a leftover root `.hermes/` (exemption removed); guard still blocks tool writes to `.hermes/**`; `/dir-whip list` shows the quarantine line (5.18/5.7 R5) | TestSettleSelfheal / TestConfigSurface / TestAuditWorkspace | 43.R5.1 | Done (2026-08-30) |
| 6 | Gate zero-delete contract: `--gate` never deletes (pre-planted expired `.tmp` session dirs untouched); wakeAgent payload EXACTLY two keys {wakeAgent, violations}; gate+json stdout = exactly two JSON lines; interactive mode lists expired entries as a read-only proposal (no "cron mode auto-cleans" wording); H1 unresolved-root exit 2 + interactive fail-open regression; symlinked session dirs / symlinked `.tmp` are not listed by the inventory. SCR-042 assertion map: :536/:547 two-key rewrite, :560 two-line rewrite, :607 wording rewrite, :430/:443 inventory rewrite, :626/:633 rewrite-or-retire, :578/:457/:733 retired (3.4/4.2/7.2 R6) | TestAuditWorkspace (extended) | 43.R6.1 | Done (2026-08-30) |
| 7 | Full suite green at baseline (687 passed / 5 skipped re-measured at 42.9; SCR-043 additions and rewrites on top) | full suite | 43.9 | Done (2026-08-30: 710 passed / 5 skipped / 0 failed) |
