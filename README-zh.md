...
workspace-guard 为 Hermes 智能体的工作目录（Working Directory）强制执行文件
...
- **天生可观测** —— 3 个斜杠命令、5 个 `workspace-guard:*` 事件，以及隐私
...
| **命令** | `/workspace-guard status`、`stats [--all] [--subagent]`、`doctor` |
| **工具** | `workspace_guard_allow_path` —— 会话级路径豁免，插件唯一工具 |
| **配置** | `guard-config.yaml` 共 4 个键，位于插件目录之外，重装不受影响 |
| **治理** | `audit_workspace.py --gate` 配合 cron 的 wakeAgent / [SILENT] 模式 |
| **可观测性** | stats.jsonl（5 MB 滚动、隐私裁剪）；5 个 `workspace-guard:*` 事件 |
...
> **面向 AI 助手的提示：** 如果你是受用户委托安装或验证 workspace-guard 的
...
如需完整的纪律参考，用 `workspace-guard:workspace-organization` 显式加载
捆绑技能。
...
| 命令 | 用途 |
| ---- | ---- |
| `/workspace-guard status` | 生效配置 + 解析来源（working_dir_root、terminal_guard、exempt_paths、allowed_root_files） |
| `/workspace-guard stats [--all] [--subagent]` | 本会话的拦截统计；`--all` 读取持久化汇总 |
| `/workspace-guard doctor` | 配置自检：可解析性、各键、解析链、统计可写性 |

`workspace_guard_allow_path(path)` 是插件唯一的工具：为当前会话注册用户指定
的路径（见高级用法）。

## 配置

可选、由用户维护，位于 `HERMES_HOME/workspace-guard/guard-config.yaml`
...
**工作目录解析。** 解析分三步：guard-config.yaml 中显式 `working_dir_root`
优先；否则取当前档案的 `terminal.cwd`；两者皆不可用时回退到当前工作目录并
发出 WARNING。`/workspace-guard status` 会报告取值及其来源。

**会话级路径豁免。** 当用户在对话中明确指定目标路径时，写入前先调用
`workspace_guard_allow_path(path)`。该记录仅对当前会话有效，并与
`exempt_paths` 合并。

**统计。** 每次守卫判定以一行 JSON 追加写入
`HERMES_HOME/workspace-guard/stats.jsonl` —— 不含文件内容、绝对路径或提示词
文本。超过 5 MB 时滚动为 `stats.jsonl.1`。跨会话汇总用
`/workspace-guard stats --all`。
...
workspace-guard 是纪律辅助工具，不是安全边界。
...
裁剪。`--workspace` 不匹配时审计门拒绝唤醒 agent，`/workspace-guard doctor`
可核验配置与统计健康。
...
目录并重新定向 —— 绝不绕过守卫。定期查看 `/workspace-guard stats`。
