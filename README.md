...
- **Windows terminal** (PowerShell/CMD/Windows Terminal, `bash install.sh` —
  ...
  (`%LOCALAPPDATA%\hermes`).
- **WSL** (a real WSL session, launched inside WSL): installs into the WSL
  Hermes (`~/.hermes`) only. If the WSL side has no Hermes, the script fails
  with guidance — it never falls back to the Windows Hermes automatically.
...
Override the routing with `WG_TARGET=wsl` (force the WSL side) or
...
**Network**: the latest skill/plugin versions shown by `status` (the
`latest version skill: ... | plugin: ...` row) are fetched from the GitHub
remote (`raw.githubusercontent.com`) — the script works standalone, no repo
checkout needed next to it. `install` also needs the remote (version comparison
+ config template download): offline it fails with a
"connect to the network and retry" hint instead of silently skipping updates;
`status` then shows `-` for the latest versions (installed versions still
shown); `uninstall` never needs the network.
